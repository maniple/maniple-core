...
/**
 * Polish translation for Zend_Validate
 *
 * Taken from https://github.com/mthenw/Zend_Validate-Polish
 */
return array(
    // Zend_Validate_Alnum
    ...
    "'%value%' contains characters which are non alphabetic and no digits" => "Wartość '%value%' powinna zawierać tylko litery i cyfry",
    ...
    "Luhn algorithm (mod-10 checksum) failed on '%value%'" => "Błąd podczas wykonywania algorytmu Luhna (suma kontrolna modulo 10) dla wartości '%value%'",
    ...
    "Invalid type given. String expected" => "Podana wartość nie jest ciągiem znaków",
    "'%value%' is no valid email address in the basic format local-part@hostname" => "'%value%' nie jest poprawnym adresem e-mail w formacie nazwa@serwer",
    "'%hostname%' is no valid hostname for email address '%value%'" => "Adres e-mail '%value%' zawiera niepoprawną nazwę serwera '%hostname%'",
    ...
    "Minimum expected width for image '%value%' should be '%minwidth%' but '%width%' detected" => "Plik '%value%' ma szerokość '%width%'. Minimalna szerokość to '%minwidth%'",
    ...
    "File '%value%' was only partially uploaded" => "Plik '%value%' został wysłany częściowo",
